...
function probe(): Promise<HealthInfo> {
  return new Promise((resolve) => {
    ...
    try {
      ...
      // so if `claude --version` output arrives across multiple chunks the
      // promise can resolve with an empty stdout. "close" only fires after all
      // stdio streams are flushed. Matches the pattern already used in runs.ts.
      proc.on("close", (code) => {
        if (code === 0) {
